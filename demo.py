...
from src.video_builder import write_video, write_comparison_video, export_frame_pngs
...
DEMO_GSD_M_PER_PX = 3.0   # simulated Sentinel-2 equivalent (metres per pixel)
...
def generate_synthetic_frames(config: dict = SYNTH_CONFIG) -> Tuple[List[np.ndarray], dict]:
    ...
    Returns:
        Tuple of BGR frames and ground-truth centers keyed by frame ID.
    """
    ...
    for frame_idx in range(config["num_frames"]):
        ...
        for ship in ships:
            if ship.is_visible((h, w)):
                gt[frame_idx].append((ship.cx, ship.cy))
...
def generate_dataset(
    ...
    """Generate a named synthetic satellite dataset into data/<name>/"""
    config = {**SYNTH_CONFIG, "num_ships": num_ships, "num_frames": num_frames}
    rng = np.random.default_rng(seed=seed)
    h, w = config["frame_size"]
    ships = [SyntheticShip(i, (h, w), rng) for i in range(num_ships)]
    frames = []
    ...
    for i, frame in enumerate(frames, start=1):
        cv2.imwrite(str(out_dir / f"frame_{i:03d}.png"), frame)
    print(f"Dataset '{name}' saved → {out_dir}/  ({num_frames} frames, {num_ships} ships)")


# ── Reporting ─────────────────────────────────────────────────────────────────

def _write_tracking_report(all_tracks: list, output_path: str, fps: int, gsd_m_per_px: float) -> None:
    ...
    with open(output_path, "w") as fh:
        ...
        for track in sorted(all_tracks, key=lambda t: t.id):
            ...
            fh.write(f"  Avg speed     : {track.speed():.2f} px/frame  |  "
                     f"{track.speed_knots(gsd_m_per_px, fps):.2f} knots\n")
...
def run_demo() -> None:
    ...
    raw_frames, gt = generate_synthetic_frames(SYNTH_CONFIG)
    ...
    print(f"  Avg center error   : {metrics['avg_center_error_px']:.1f} px")
    print(f"  TP / FP / FN       : {metrics['TP']} / {metrics['FP']} / {metrics['FN']}")
    print("--------------------------\n")
    print(f"  GSD assumed    : {DEMO_GSD_M_PER_PX} m/px  (simulated Sentinel-2)\n")
    ...
    write_comparison_video(
        processed,
        annotated,
        str(OUTPUT_DIR / "demo_comparison.mp4"),
        fps=fps,
    )
    export_frame_pngs(annotated, str(OUTPUT_DIR / "frames"), prefix="annotated")

    _write_tracking_report(all_tracks, str(OUTPUT_DIR / "tracking_report.txt"), fps, DEMO_GSD_M_PER_PX)
...
if __name__ == "__main__":
    run_demo()
    generate_dataset("busy_port",  num_ships=6, num_frames=30, seed=7)
    generate_dataset("open_ocean", num_ships=2, num_frames=20, seed=42)
